...
@Component({
  selector: 'app-modal-config-payment',
  templateUrl: './modal-config-payment.component.html',
  styleUrls: ['./modal-config-payment.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class ModalConfigPaymentComponent implements OnInit {
  ...
  //Special Conditions
  modelSpecialCondition: SpecialConditionSupplier;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalConfigPaymentComponent>,
  ...
  savePercent() {
    if (!this.configForm.valid) {
      ...
      let model: PaymentConditionSupplier = {
        paymentConditionId: this.data.paymentConditionId,
        paymentConditionSupplierId: this.data.id,
    ...
    this.itemMenusSpecialCondition = [
      {
        ...
        action: (row: SpecialConditionDto) => {
          let sub = this._service
            .deleteSpecialConditionSupplier(row.id)
            .subscribe((result) => {
              if (result.succeeded) {
                this.persistenceDb = true;
  ...
  formSpecialConditionSubmit() {
    ...
    if (!form) {
      ...
      let model: SpecialConditionSupplier = Object.assign(
        new SpecialConditionSupplier(),
        this.createFormSpecialCondition(),
        form.getRawValue()
      );
      ...
      model.paymentConditionSupplierId = this.data.id;
      model.restrictConditionType = this.specialConditionType;

      this._service.addSpecialConditionSupplier(model).subscribe((result) => {
        if (result.succeeded) {
          this.persistenceDb = true;
