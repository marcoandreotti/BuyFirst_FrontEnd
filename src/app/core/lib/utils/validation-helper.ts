export class ValidationHelper {
    ...
    static validaCpfOuCnpj(cpfOuCnpj: string): boolean {
        let valido = false;
        if (cpfOuCnpj?.length === 11) valido = this.validaCpf(cpfOuCnpj);
        else if (cpfOuCnpj?.length === 14) valido = this.validaCnpj(cpfOuCnpj);
        return valido;
    }
}